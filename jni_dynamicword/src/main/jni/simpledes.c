...
int des_encrypt(unsigned char *source, int srcLen, unsigned char *dest, unsigned char *key)
{
	...
	int nPos = 0; 
	while (nPos < srcLen) {
		int nLeaveSize = srcLen - nPos;
		if (nLeaveSize >= 8)
		{
			encrypt0(source + nPos, dest + nPos); 
			nPos += 8;
		} else {
			unsigned char szTmp[8] = {0}; 
			memcpy(szTmp, source + nPos, nLeaveSize);
			unsigned char c = '\0' + 8 - nLeaveSize;
			printf("leave:%d  cover:%d\n", nLeaveSize, c);
			int i;
			//for (i = nLeaveSize; i < 8; i ++)
			//	szTmp[i] = c;
			//szTmp[7] = nLeaveSize;
			encrypt0(szTmp, dest + nPos); 
			nPos += nLeaveSize;
		}
	} 
	return 0;
