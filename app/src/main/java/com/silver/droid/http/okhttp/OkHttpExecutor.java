...
package com.silver.droid.http.okhttp;
...
import com.silver.droid.http.common.HttpResult;
import com.silver.droid.http.common.RequestConfig;
import com.silver.droid.http.common.RequestExecutor;
import com.silver.droid.http.common.RequestMethod;
...
//    public void getStringFromServerByModel(Callback callback){
//
//        String baseUrl = Const.MAKE_REQUEST_URL(module);
//
//        allParams.putAll(XAuthSign.generateURLParams(method, baseUrl, allParams,
...
//
//        String resUrl = baseUrl + "?" + encodeParameters(allParams, RequestConfig.PARAMS_ENCODING);
    ...
    public Observable<HttpResult> oKObservable() {
        ...
        return Observable.defer(new Func0<Observable<HttpResult>>() {
            @Override public Observable<HttpResult> call() {
                ...
                try {
                    beforeExecute();
                    Response response = mOkHttpClient.newCall(requestBuilder.url(requestUrl).build()).execute();
                    JsonObject jsonObject = new Gson().fromJson(response.body().charStream(), JsonObject.class);
                    taskResult.parse(jsonObject);
                }catch (Exception ex){
